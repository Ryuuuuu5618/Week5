...
static char *skip_ws(char *s) {
    while (*s == ' ' || *s == '\t') s++;
    return s; // 0x7fffffffe026 "example.com"
}
...
static void parse_headers(char *text, Headers *h) {
    ...
    for (char *line = strtok(text, "\n"); line != NULL; line = strtok(NULL, "\n")) {
        ...
        *colon = '\0';
        ...
        // "Host: example.com\n" -> "Host\n example.com\n"
        char *key = line; // 0x7fffffffe020 "Host"
        char *val = skip_ws(colon + 1); // 0x7fffffffe026 "example.com"

        // h->count = 0;
        if (h->count < MAX_HEADERS) {
            h->keys[h->count] = key; // 0x7fffffffe020 "Host"
            h->vals[h->count] = val; // 0x7fffffffe026 "example.com"
            h->count++; // h->count = 1
        }
    }
}
